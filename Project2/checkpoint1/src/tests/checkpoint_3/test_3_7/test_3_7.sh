...
function execute_part3_tests()
{
   ...
   echo "Copying file"
   chmod 777 $TEST_DIR/largefile
   cp $TEST_DIR/largefile $TESTDIR
   cp $TEST_DIR/largefile $TEMPDIR
   sleep 1

   chmod 444 $TESTDIR/largefile
   chmod 444 $TEMPDIR/largefile 
   ...
   print_result $?
       ...
       sleep 1
    echo -ne "Checking for time integrity           "
    (cd $TEMPDIR && stat $TESTDIR/largefile > $LOG_DIR/expected.txt)
    (cd $TESTDIR && stat $TEMPDIR/largefile > $LOG_DIR/real.txt)
    diff $LOG_DIR/expected.txt $LOG_DIR/real.txt
    print_result $?
