...
void create(const char *filename, const char *folderpath) {
  ...
  for (;;) {
    ...
    chmod(archive_entry_sourcepath(entry), old_mode|S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
...
void extract(const char *filename, const char *foldername) {
  ...
  std::unordered_map<std::string, mode_t> mode_map;
	a = archive_read_new();
    ...
    const struct stat *statbuf = archive_entry_stat(entry);
    log_msg(logfile, "debug read_only_file\n");
    mode_t old_mode = statbuf->st_mode;
    log_msg(logfile, "debug oldmode %x\n", old_mode);
    mode_map[std::string(fpath_for_folder) + archive_entry_pathname(entry)] = old_mode;
    archive_entry_set_pathname(entry, (std::string(fpath_for_folder) + "/" + archive_entry_pathname(entry)).c_str());
    archive_entry_set_mode(entry, old_mode|S_IRWXU|S_IRWXG|S_IRWXO);
  ...
  for (std::unordered_map<std::string, mode_t>::iterator iter = mode_map.begin(); iter != mode_map.end(); iter++) {
    int ans = chmod(iter->first.c_str(), iter->second);
    if (ans < 0) {
      log_msg(logfile, "chmod %s, %x error\n", iter->first.c_str(), iter->second);
    } else {
      log_msg(logfile, "chmod %s, %x success\n", iter->first.c_str(), iter->second);
    }
  }
}
