...
{
	OPTIONS=`getopt -o s:f:a:t:m:S:M:w:c:do --long ssd-path:,fuse-path:,ssd-size:,threshold:,min-seg-size:,avg-seg-size:,max-seg-size:,rabin-window-size:,cache-size:,no-dedup,no-cache -n '$0' -- "$@"`
	...
	while true; do
		case "$1" in
		-s|--ssd-path) CLOUDFSOPTS+=" $1 $2"; SSDPATH=$2; shift 2;;
		-f|--fuse-path) CLOUDFSOPTS+=" $1 $2"; FUSEPATH=$2; shift 2;;
		-a|--ssd-size) CLOUDFSOPTS+=" $1 $2"; SSDSIZE=$2; shift 2;;
		-t|--threshold) CLOUDFSOPTS+=" $1 $2";THRESHOLD=$2; shift 2;;
		-m|--min-seg-size) CLOUDFSOPTS+=" $1 $2"; MINSEGSIZE=$2; shift 2;;
		-S|--avg-seg-size) CLOUDFSOPTS+=" $1 $2"; AVGSEGSIZE=$2; shift 2;;
		-M|--max-seg-size) CLOUDFSOPTS+=" $1 $2"; MAXSEGSIZE=$2; shift 2;;
...
{
  MAX_CACHE_SIZE=1048576 # 1 MB
  if [ -d "$CACHEDIR" ]; then
    cache_size=$(du -b $CACHEDIR | cut -f 1)
    if [ "$cache_size" -gt $MAX_CACHE_SIZE ]; then
      echo "Cache size larger than max allowed size of $MAX_CACHE_SIZE"
      print_result 1
      exit
    fi
  fi
...
function check_cache_size_params()
{
  MAX_CACHE_SIZE=$(($1 * 1024))
  if [ -d "$CACHE_DIR" ]; then
    cache_dir_size=$(stat -c '%s' $CACHE_DIR)
    cache_size=$(($(du -b $CACHE_DIR | cut -f 1) - $cache_dir_size))
