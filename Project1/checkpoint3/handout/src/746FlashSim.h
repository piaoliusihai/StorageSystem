...
#ifndef __746FLASHSIM_H__
#define __746FLASHSIM_H__
...
/* Forward declaration */
template <typename PageType> class FlashSimExecCallBack;

class FlashSimTest;

IPC_Format send_req_to_ftl(class FlashSimTest *fs_test,
					IPC_Format *tx_msg);

template <typename PageType> class FlashSimFTL;
...
class FlashSimConf : public ConfBase {
	...
	FlashSimConf(const std::string &p_file_name) :
		...
		while(fp) {
			...
			std::getline(fp, line);
        		...
        		size_t start_index =
				line.find_first_not_of(space_pattern);
			 ...
			 * and value substring
		 	 */
        		size_t end_index =
				line.find_first_of(space_pattern, start_index);
			 ...
			 */
        		std::string key = line.substr(start_index,
						end_index - start_index);

        		/* Then find the value */

        		start_index = line.find_first_not_of(space_pattern,
								end_index);
			 ...
			 * line which should also cause this branch being taken
	 		 */
        		...
        		end_index = line.find_first_of(space_pattern,
								start_index);
			 ...
			 */
        		if(configuration_map.find(key) !=
				configuration_map.end()) {
          			ThrowKeyAlreadyExistsError(key);
        		}

        		configuration_map[key] =
				std::make_pair(value, line_num);
		} /* while fp is not EOF */
	...
	size_t GetSSDSize(void) const {
		return (size_t)GetInteger(CONF_S_SSD_SIZE);
	}
	...
	size_t GetDieSize(void) const {
		return (size_t)GetInteger(CONF_S_DIE_SIZE);
	}
	...
	void ThrowFileNotFoundError() const {
		...
		"The given configuration file " + file_name +
		" could not be found!");
	}
	...
	void ThrowKeyAlreadyExistsError(const std::string &key) const {
		...
		throw FlashSimException("Key \"" + key +
...
class DataStore {
	...
	void EraseRange(size_t start_slot_id, size_t end_slot_id) {
      		...
      		/* start < end && end < count */
		if ((start_slot_id > end_slot_id) ||
			(end_slot_id >= slot_count)) {
       		 	ThrowErasingInvalidRangeError(start_slot_id,
							end_slot_id);
      		}

      		for (size_t slot_id = start_slot_id;
			slot_id <= end_slot_id; slot_id++) {
	...
	void ThrowSparseFileNotSupportedError() const {
		throw FlashSimException("Sparse file is not supported"
	 ...
	 */
	 void ThrowSlotOutOfBoundError(size_t slot_id) const {
		throw FlashSimException(std::string{"Slot "} +
	...
	void ThrowErasingInvalidRangeError(size_t start_slot_id,
		...
		throw FlashSimException(std::string{"The range ["} +
...
class Controller {
...
#if ENABLE_TRANS_TRACING
			fprintf(trans_trace_fp, "W 1 %zu <%d,%d,%d>\n", logical_lba, addr.plane, addr.block, addr.page);
#endif
...
#else
		auto ret = ftl_p->ReadTranslate(lba,
					FlashSimExecCallBack<PageType>(this));
#endif
...
#else
		auto ret = ftl_p->WriteTranslate(lba,
					FlashSimExecCallBack<PageType>(this));
#endif
		/* Make sure nothing is left in page buffer after translation */
		EnsureStateIsClean();
	...
	*/
	uint64_t TotalOps(OpCode code) {
        ...
        bool AtLeastOneBlockWornOut() {
                ...
                for (auto it = block_erasure_map.begin();
                     it != block_erasure_map.end(); ++it) {
                        if (it->second == 0) {
                                return true;
                        }
                }
	...
	void UpdateBlockErasure(size_t block_lba) {
      		...
      		/*
       		 ...
       		 */
      		const auto &value =
        		std::make_pair(block_lba, block_erase_count);
	...
	*/
	...
	void ThrowWriteDirtyPageError(size_t physical_lba) {
		...
		throw FlashSimException(
		"Write operation on dirty physical page " +
	...
	void ThrowInvalidReadError(size_t physical_lba) {
		...
		throw FlashSimException(
		"Read operation on invalid physical page " +
		std::to_string(physical_lba));
	}
...
class FlashSimTest {
	...
	int Write(FILE* log, size_t addr, const TEST_PAGE_TYPE &buf) {
		...
		try {
			...
			status = ctrl.WriteLBA(buf, addr);
      			...
      			std::cout << "!!! Error writing LBA " << addr <<
		...
		if (status != ExecState::SUCCESS) {
			...
			if (log)
				fprintf(log, "LBA %zu not writable\n", addr);
			return 0;
		} else {

			writes_done++;
			if (log)
				fprintf(log, "LBA %zu written\n", addr);
	...
	int Read(FILE* log, size_t addr, TEST_PAGE_TYPE* buf) {
		...
		try {
			...
			std::cout << "!!! Error reading LBA " << addr <<
				" !!!" << std::endl << err.what();
			return -1;
		}
		...
		if (status != ExecState::SUCCESS) {
			if (log)
				fprintf(log, "LBA %zu not readable\n", addr);
			return 0;
		} else {
			if (log)
				fprintf(log, "LBA %zu read\n", addr);
	...
	int Trim(FILE* log, size_t addr) {
		...
		try {
			...
			status = ctrl.Trim(addr);
      			...
      			std::cout << "!!! Error trimming LBA " << addr << " !!!"
			<< std::endl << err.what();
      			return -1;
    		}
		...
		if (status != ExecState::SUCCESS) {
			...
			if (log)
				fprintf(log, "LBA %zu not trimmed\n", addr);
	...
	int Report(FILE* log) {
		...
		size_t mem_usage = 1;
		printf("WARNING: Memory check disabled."
		...
		if (is_inf)
		{
                        endurance_score = (double)conf.GetWeightEnduranceInfinite() *
                            (MIN((writes_done * conf.GetWritesThreshold() /
                                 ...
                                 1.0));
                        amp_score = (double)conf.GetWeightWriteAmplificationInfinite() *
                            (MIN((double)conf.GetWriteAmplificationThreshold() / write_amp,
                                 1.0));
    		...
    		else
    		{
     			...
     			endurance_score = 0;
      			amp_score = (double)conf.GetWeightWriteAmplificationFinite() *
	...
	uint64_t TotalErasesPerformed() {
                return ctrl.TotalOps(OpCode::ERASE);
	}
	...
	uint64_t TotalWritesPerformed() {
                return ctrl.TotalOps(OpCode::WRITE);
	}
         ...
         */
        bool AtLeastOneBlockWornOut() {
                return ctrl.AtLeastOneBlockWornOut();
        }
...
class FlashSimFTL : public FTLBase<PageType> {
	...
	FlashSimFTL(FlashSimTest *fs_test):
		fs_test(fs_test) {
	};
	...
	ReadTranslate(size_t lba, const ExecCallBack<PageType> &) {

		IPC_Format tx_msg, rx_msg;

		memset(&tx_msg, 0, sizeof(tx_msg));

		tx_msg.owner = OWNER_FLASHSIM;
		tx_msg.type = MSG_FTL_INSTR_READ;
		tx_msg.lba = lba;

		/* Send the IPC message to FTL and get response */
		SendReqToFtl(&tx_msg, &rx_msg);

		return std::make_pair(rx_msg.ftl_resp_execstate,
					rx_msg.ftl_resp_addr);

	}

    	std::pair<ExecState, Address>
    	WriteTranslate(size_t lba, const ExecCallBack<PageType> &) {
		...
		tx_msg.owner = OWNER_FLASHSIM;
		tx_msg.type = MSG_FTL_INSTR_WRITE;
		tx_msg.lba = lba;
    	...
    	ExecState
    	Trim(size_t lba, const ExecCallBack<PageType> &) {
		...
		tx_msg.owner = OWNER_FLASHSIM;
		tx_msg.type = MSG_FTL_INSTR_TRIM;
		tx_msg.lba = lba;

		/* Send the IPC message to FTL and get response */
		SendReqToFtl(&tx_msg, &rx_msg);

		return rx_msg.ftl_resp_execstate;
	...
	size_t GetFTLStackSize(void) {
		...
		tx_msg.owner = OWNER_FLASHSIM;
		tx_msg.type = MSG_FTL_STACK_SIZE_REQ;

		/* Send the IPC message to FTL and get response */
		SendReqToFtl(&tx_msg, &rx_msg);

		return rx_msg.child_stack_size;
	 ...
	 */
	void RecvMsgFromFtl(IPC_Format *rx_msg,
					int should_block) {
		...
		if (ret == 0) {
			...
			assert(should_block == 0 &&
				"No data even on blocking poll");
			rx_msg->type = MSG_EMPTY;
			return;
			...
			assert(rx_msg->owner == OWNER_FTL && "Unknown owner");
			return;
		}
	}
	...
	void SendMsgToFtl(IPC_Format *tx_msg) {
		...
		assert((tx_msg->owner == OWNER_FLASHSIM) && "Unknown owner");
	 ...
	 * response from FTL
	 */
	void ProcessRequests(IPC_Format *recv_msg,
					int should_block)
	{
		IPC_Format send_msg;
		...
		while (1) {
			...
			case MSG_CONF_REQ_SSD_SIZE:
				...
				send_msg.type = MSG_CONF_RES_SSD_SIZE;
				send_msg.conf_resp = fs_test->conf.GetSSDSize();
				break;

			case MSG_CONF_REQ_PACKAGE_SIZE:

				send_msg.type = MSG_CONF_RES_PACKAGE_SIZE;
				send_msg.conf_resp =
					fs_test->conf.GetPackageSize();
				break;

			case MSG_CONF_REQ_DIE_SIZE:

				send_msg.type = MSG_CONF_RES_DIE_SIZE;
				send_msg.conf_resp = fs_test->conf.GetDieSize();
				break;

			case MSG_CONF_REQ_PLANE_SIZE:

				send_msg.type = MSG_CONF_RES_PLANE_SIZE;
				send_msg.conf_resp =
					fs_test->conf.GetPlaneSize();
				break;

			case MSG_CONF_REQ_BLOCK_SIZE:

				send_msg.type = MSG_CONF_RES_BLOCK_SIZE;
				send_msg.conf_resp =
					fs_test->conf.GetBlockSize();
				break;

			case MSG_CONF_REQ_BLOCK_ERASES:

				send_msg.type = MSG_CONF_RES_BLOCK_ERASES;
				send_msg.conf_resp =
					fs_test->conf.GetBlockEraseCount();
				break;

			case MSG_CONF_REQ_OVERPROVISIONING:

				send_msg.type=
					MSG_CONF_RES_OVERPROVISIONING;
				send_msg.conf_resp =
					fs_test->conf.GetOverprovisioning();
				break;

			case MSG_CONF_REQ_GCPOLICY:

				send_msg.type=
					MSG_CONF_RES_GCPOLICY;
				send_msg.conf_resp =
					fs_test->conf.GetGCPolicy();
				break;
			...
			case MSG_SIM_REQ_ERASE: /* Fall through */
				...
				sim_req_opcode = recv_msg->sim_req_opcode;
				sim_req_addr = recv_msg->sim_req_addr;

				fs_test->ctrl.ExecuteCommand(sim_req_opcode,
								sim_req_addr);

				send_msg.type = MSG_EMPTY;
				break;
	 ...
	 */
	void  SendReqToFtl(IPC_Format *tx_msg,
				IPC_Format *rx_msg) {

		/* Expected message type of the response to msg transmitted */
		...
		while (1) {
			...
			if (rx_msg->type == MSG_EMPTY)
				continue;
			else if (rx_msg->type == exp_rx_typ)
				break;
			else
...
/*************************** class FlashSimFTL ends ***************************/

#endif /* __746FLASHSIM_H__ */
